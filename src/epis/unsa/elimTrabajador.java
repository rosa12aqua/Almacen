...
public class elimTrabajador extends HttpServlet{
	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		System.out.println(String.valueOf(elejidos.length+" trabajador"));
		...
		try{
			if( req.getParameterValues("elegidos")!=null )
				for(int i=0;i<elejidos.length;i++){
					pm.deletePersistent(pm.getObjectById(Trabajador.class, Long.parseLong(elejidos[i])));
				}				
		}catch(Exception e){
